                          ...
                          )
{
    ...
    do 
    {
        ...
        if(row_ind.size() < 3)
        {
            while(remaining_col_ind.size() != 0 &&
        ...
        else
        {
            ...
            std::vector<int> row_ind_mid(row_ind_sort.size() - 1);

            for(int i = 0; i < row_ind_sort.size() - 1; i++)
            {
                row_ind_diff[i] = row_ind_sort[i+1] - row_ind_sort[i];
                row_ind_mid[i]  = row_ind_sort[i]   + row_ind_diff[i] / 2;
            }
            ...
            {
                row_ind.pop_back();
                new_row_ind = row_ind_mid.back();
                row_ind_mid.pop_back();
                row_ind.push_back(new_row_ind);
                remaining_row_ind.erase(new_row_ind);
                // Generation of the row
                // Row of the residuum and the pivot column
        ...
        else
        {
            ...
            std::vector<int> col_ind_mid(col_ind_sort.size() - 1);

            for(int i = 0; i < col_ind_sort.size() - 1; i++)
            {
                col_ind_diff[i] = col_ind_sort[i+1] - col_ind_sort[i];
                col_ind_mid[i]  = col_ind_sort[i]   + col_ind_diff[i] / 2;
            }
            ...
            {
                col_ind.pop_back();
                new_col_ind = col_ind_mid.back();
                col_ind_mid.pop_back();
                col_ind.push_back(new_col_ind);
                remaining_col_ind.erase(new_col_ind);
