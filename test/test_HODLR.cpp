...
int main(int argc, char* argv[]) 
{
    ...
    testHODLR(N, n_levels, tolerance, K_gaussian);
    delete K_gaussian;

    // This is a tough check of the low rank approximation, we set a lower tolerance for this:
    tolerance  = pow(10, -7);
    Random_Matrix* random_matrix = new Random_Matrix(N);
    testHODLR(N, n_levels, tolerance, random_matrix);
